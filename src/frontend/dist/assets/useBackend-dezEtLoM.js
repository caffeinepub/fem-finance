...
Service({
  "getContactMessages": Func([], [Vec(ContactMessage)], ["query"]),
...
const idlFactory = ({ IDL: IDL2 }) => {
  ...
  return IDL2.Service({
    "getContactMessages": IDL2.Func([], [IDL2.Vec(ContactMessage2)], ["query"]),
    ...
    "submitLead": IDL2.Func([LeadInput2], [Lead2], [])
  });
};
...
class Backend {
  ...
  async submitLead(arg0) {
    if (this.processError) {
      try {
        const result = await this.actor.submitLead(to_candid_LeadInput_n5(this._uploadFile, this._downloadFile, arg0));
...
function from_candid_Lead_n2(_uploadFile, _downloadFile, value) {
...
function from_candid_record_n3(_uploadFile, _downloadFile, value) {
  return {
    ...
    serviceTag: record_opt_to_undefined(from_candid_opt_n4(_uploadFile, _downloadFile, value.serviceTag)),
    phone: value.phone
  };
}
...
function to_candid_LeadInput_n5(_uploadFile, _downloadFile, value) {
...
function to_candid_record_n6(_uploadFile, _downloadFile, value) {
  return {
    city: value.city,
    name: value.name,
    serviceTag: value.serviceTag ? candid_some(value.serviceTag) : candid_none(),
    phone: value.phone
  };
}
...
function useBackend() {
  ...
  const submitContactMessage = async (input) => {
    if (!actor || isFetching) return false;
    try {
      const backendActor = actor;
